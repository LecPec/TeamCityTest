...
void particle_leave(Particles &particles, const Grid &grid, const scalar Radius, const array<scalar, 2> circle_center, scalar dt, const Matrix& rho) {
    int ptcl_idx = 0;
    int row = 0, col = 0; 
    while (ptcl_idx < particles.get_Ntot()) {
        row = floor(particles.y[ptcl_idx] / grid.dy);
        col = floor(particles.x[ptcl_idx] / grid.dx);
        if (pow(particles.x[ptcl_idx] - circle_center[0], 2) +
            pow(particles.y[ptcl_idx] - circle_center[1], 2) >= pow(Radius, 2)) {
            //particles.update_ptcls_on_anode_leave(row, col);
            //particles.UpdateNumOfPtclsOnAnode(grid, rho, ptcl_idx); Почему-то после mpi сломалась!!!
            //cout << "leaving" << endl;
            particles.pop(ptcl_idx);
        }
        else {
