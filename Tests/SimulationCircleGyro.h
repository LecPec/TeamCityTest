//
// Created by Vladimir Smirnov on 17.11.2021.
//

#ifndef CPP_2D_PIC_SIMULATIONCIRCLEGYRO_H
#define CPP_2D_PIC_SIMULATIONCIRCLEGYRO_H
...
template<class Ptcls>
int anode_current(Ptcls& particles, scalar Radius, const array<scalar, 2> circle_center) {
    int Ntot = 0;
...
template<class Ptcls>
array<scalar, 3> mean_energy_xyz(Ptcls& particles) {
...
void test_simulation_circle_gyro() {
    ...
    cout << "debye radius: " << pow(EPSILON_0 * EV * 100 / (EV*EV*10*init_dens), 0.5) << endl;
    /*****************************************************/
    // Particle Init
    scalar m_e = E_M;
    ...
    scalar gamma = 0.5; //secondary emission coefficient
    scalar ion_radius_leave = 0.1 * Nx * dx;
    ...
    int ion_step = sqrt(m_ion/m_e)/gyro_coeff;
    ...
    int Ntot_step = 1000, energy_step = Ntot_step, pos_step = it_num/1000, vel_step = it_num/1000;
    ...
    cout << "simulation gyro finish" << endl;
}


#endif //CPP_2D_PIC_SIMULATIONCIRCLEGYRO_H
